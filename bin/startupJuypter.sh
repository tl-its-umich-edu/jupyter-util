...
## Copy dockefile since it must be in local context
#cp ../bin/Dockerfile .

IMG_TAG=myownprivatejupyter

# Always rebuild the container to get the most recent version.
#docker build -t ${IMG_TAG} .
cat ../bin/Dockerfile | docker build -t ${IMG_TAG} -

# The ..VOL variables below show the Docker syntax to map an
# in-container and an external directory. This argument can be
# supplied multiple times.  If the local directory doesn't already
# exist is it created.
